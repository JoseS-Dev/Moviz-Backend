...
export class UsersService {
  ...
  async create(createUserDto: CreateUserDto): Promise<User> {
    if(!createUserDto) throw new BadRequestException("Los datos de creación del usuario son requeridos.");
  ...
  async findAll(page: number = 1, limit: number = 10): Promise<{ users: User[], total: number }> {
    ...
    if(skip >= totalUsers) throw new BadRequestException("No se encontraron usuarios para la página solicitada.");
  ...
  async findByEmail(email: string): Promise<User> {
    ...
    if(!user) throw new BadRequestException(`No se encontró el usuario con email ${email}.`);
    return user;
  }
  ...
  async update(id: number, updateUserDto: UpdateUserDto): Promise<User> {
    ...
    if(!existingUser) throw new BadRequestException(`No se encontró el usuario con ID ${id}.`);
    // Si se va actualizar le correo, se verifica que no exista otro usuario con ese correo
    if(updateUserDto.email && updateUserDto.email !== existingUser.email) {
      const emailExists = await this.findByEmail(updateUserDto.email);
      if(emailExists) throw new BadRequestException("El email ya está registrado por otro usuario.");
    }
    // Si se va actualizar la contraseña, se hashea antes de guardarla
    if(updateUserDto.password_hash) {
      updateUserDto.password_hash = await bcryptjs.hash(updateUserDto.password_hash, 10);
    }
  ...
  async remove(id: number): Promise<User> {
    ...
    if(!deleted) throw new BadRequestException("No se pudo eliminar el usuario.");
    return deleted;
  }
}
