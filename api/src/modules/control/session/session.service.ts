...
export class SessionService {
  ...
  async login(createSessionDto: CreateSessionDto) {
    if(!createSessionDto) throw new NotFoundException('No se propocionaron los datos de login');
    // Se verifica que el usuario exista con el gmail
    const existingUser = await this.prisma.users.findUnique({
      where: { email: createSessionDto.email }
    });
    if(!existingUser) throw new NotFoundException('No se encontró un usuario con el correo proporcionado');
    // Se verifica que la contraseña sea la correcta
    const isPasswordValid = await bcrypt.compare(createSessionDto.password, existingUser.password_hash);
    ...
    if(existingSession){
      ...
      if(!updatedSession) throw new BadRequestException('No se pudo actualizar la sesión');
      ...
      if(!newSession) throw new BadRequestException('No se pudo crear la sesión');
      return newSession;
    }
  }
  ...
  async logout(userId: number){
    ...
    if(!exisitingUser) throw new NotFoundException('No se encontró un usuario con el ID proporcionado');
    // Se verifica si existe la sessión del usuario
    const existingSession = await this.prisma.sessions.findFirst({
      where: { user_id: userId }
    });
    if(!existingSession) throw new NotFoundException('No se encontró una sesión para el usuario proporcionado');
    // Si existe la sesión se elimina el token
    const deletedSession = await this.prisma.sessions.update({
      where: { id: existingSession.id },
      data: { token: null }
    });
    if(!deletedSession) throw new BadRequestException('No se pudo cerrar la sesión');
