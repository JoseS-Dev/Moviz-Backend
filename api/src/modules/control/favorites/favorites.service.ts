...
export class FavoritesService {
  ...
  async create(createFavoriteDto: CreateFavoriteDto) {
    ...
    if (!existingUser)
      throw new NotFoundException('El usuario en cuestión no existen');
    if (!existingFootage)
      throw new NotFoundException('La pelicula en cuestión no existen');
    ...
    if (!newFavorite)
      throw new BadRequestException(
        'No se pudo crear el favorito entre el usuario y la pelicula',
      );
    return newFavorite;
  }
  ...
  async findAll(userId: number, page: number = 1, limit: number = 10) {
    if (!userId)
      throw new BadRequestException('El ID del usuario no fue propocionado');
    // Se verifica que exista el usuario
    const existingUser = await this.prisma.users.findUnique({
      where: { id: userId },
    });
    if (!existingUser)
      throw new NotFoundException('No se encontro el usuario en cuestión');
  ...
  async update(id: number, updateFavoriteDto: UpdateFavoriteDto) {
    if (!id)
      throw new BadRequestException(
        'El ID del favorito a actualizar no se propociono',
      );
    if (!updateFavoriteDto)
      throw new BadRequestException('Los datos a actuaizar no se propocionado');
    // Se verifica que existe el favorito
    const existingFavorite = await this.prisma.favoriteMovies.findUnique({
      where: { id },
    });
    if (!existingFavorite)
      throw new NotFoundException('No existe el favorito a actualizar');
    ...
    if (!updatedFavorite)
      throw new BadRequestException('No se pudo actualizar el favorito');
