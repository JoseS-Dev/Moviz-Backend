...
export class FootageService {
  ...
  async create(createFootageDto: CreateFootageDto): Promise<Footage> {
    if(!createFootageDto) throw new BadGatewayException('Los datos no fueron propocionados para crear una pelicula');
    ...
    if(existingFootage) throw new ConflictException('Ya existe un pelicula con el mismo nombre');
    ...
    if(!newFootage) throw new BadRequestException('No se pudo crear la pelicula')
    return newFootage
  }

  async findAll(page: number = 1, limit: number = 10): Promise<{ footages: Footage[], total: number }> {
    const skip = (page - 1) * limit
    // Obtenemos el total de las peliculas que hay en el sistema
    const totalFootage = await this.prisma.footage.count();
    if(skip >= totalFootage) throw new BadRequestException("No se encontraron peliculas para la página solicitada.");
  ...
  async findOne(id: number): Promise<Footage> {
    ...
    if(!footage) throw new NotFoundException('No se encontro la pelicula solicitada');
    return footage
  }

  async update(id: number, updateFootageDto: UpdateFootageDto): Promise<Partial<Footage>> {
    if(!id) throw new BadRequestException('No se encontro el id de la pelicula a actualizar');
    if(!updateFootageDto) throw new BadRequestException('No se propocionaron los datos a actualizar');
    // Se verifica que exista la pelicula
    const existingFootage = await this.prisma.footage.findUnique({
      where: {id}
    });
    if(!existingFootage) throw new NotFoundException('No se encotro la pelicula a actualizar');
    ...
    if(updateFootageDto.title_footage){
      ...
      if(existingFootageWithName) throw new ConflictException('Ya existe una pelicula con el mismo nombre')
    }
    ...
    if(!updateFootageDto) throw new BadRequestException('No se pudo actualizar la pelicula')
    return updateFootageDto
  }
  ...
  async changeStatus(id: number, isActive: boolean): Promise<Footage> {
    ...
    if(!existingFootage) throw new NotFoundException('No existe la pelicula a cambiar su estado');
    // Si existe se le cambia el estado de la pelicula
    const changeStatusFootage = await this.prisma.footage.update({
      where: {id},
      data: {is_active: isActive}
    });
    if(!changeStatusFootage) throw new BadRequestException('No se pudo actualizar el estado de la pelicula');
    return changeStatusFootage 
  }

  async remove(id: number) : Promise<Footage> {
    if(!id) throw new BadRequestException('No se propociono el Id de la pelicula');
    // Se verifica la pelicula existan
    const existingFootage =  await this.findOne(id);
    if(!existingFootage) throw new NotFoundException('No se encontro la pelicula a eliminar');
    // Si existe, se elimina la pelicula
    const deletedFootage = await this.prisma.footage.delete({
      where: {id}
    });
    if(!deletedFootage) throw new BadRequestException('No se pudo eliminar la pelicula');
