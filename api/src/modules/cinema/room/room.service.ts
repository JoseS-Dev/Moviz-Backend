...
export class RoomService {
  ...
  async create(createRoomDto: CreateRoomDto) : Promise<Room> {
    if(!createRoomDto) throw new BadRequestException('Los datos no fueron propocionados');
    ...
    if(existingRoom) throw new ConflictException('Ya existe una sala con este codigo de sala');
  ...
  async findAll(page: number = 1, limit: number = 10) : Promise<{rooms: Room[], total: number}> {
  ...
  async update(id: number, updateRoomDto: UpdateRoomDto) : Promise<Room> {
    if(!id) throw new BadRequestException('No se propociono el ID de la sala a modificar');
    if(!updateRoomDto) throw new BadRequestException('Los datos a actualizar no se propocionaron');
    // Se verifica que exista la sala a modificar
    const existingRoom = await this.findOne(id);
    if(!existingRoom) throw new NotFoundException('No existe la sala que se va actualizar');
    ...
    if(!updatedRoom) throw new BadRequestException('No se pudo actualizar la sala')
      return updatedRoom
  }
  ...
  async changeStatus(id: number, isActive: boolean): Promise<Room>{
    ...
    if(!changeStatusRoom) throw new BadRequestException('No se pudo actualizar el estado de la sala');
  ...
  async remove(id: number) : Promise<Room> {
    ...
    const existingRoom =  await this.findOne(id);
    if(!existingRoom) throw new NotFoundException('No se encontro la pelicula a eliminar');
    // Si existe, se elimina la pelicula
    const deletedRoom = await this.prisma.rooms.delete({
      where: {id}
    });
    if(!deletedRoom) throw new BadRequestException('No se pudo eliminar la pelicula');
    return deletedRoom
  }
}
